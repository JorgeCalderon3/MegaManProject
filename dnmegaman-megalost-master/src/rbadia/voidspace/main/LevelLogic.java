...
public class LevelLogic {
	...
	public void handleKeysDuringPlay(InputHandler ih, LevelState levelState) {
		...
		if(ih.isUpPressed()){
			long currentTime = System.currentTimeMillis();
			if((currentTime - lastBigBulletTime) > 250){
				lastBigBulletTime = currentTime;
				for(int i=0; i<6; i++){
		...
		if(ih.isNPressed()) {
			Level1State cS = new Level1State(mute, null, status, null, ih, null, null); //skips to next level
			cS.clearScreen();
			getLevelState().setLevel(getLevelState().getLevel() + 1);
			getLevelState().doPlaying();
		}
		if (ih.isIPressed()) {
			status.setLivesLeft((status.getLivesLeft() + 1));  //Adds +1 health to character when I is pressed
			ih.reset();
		}
		if (ih.isRPressed()) {
			status.setAsteroidsDestroyed(0);	//Resets asteroids destroyed
			ih.reset();
		}
	}
